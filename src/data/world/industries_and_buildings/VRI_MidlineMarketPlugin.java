...
public class VRI_MidlineMarketPlugin extends BaseSubmarketPlugin {
    public static Logger log = Global.getLogger(VRI_MidlineMarketPlugin.class);

    public VRI_MidlineMarketPlugin() {
    }
    ...
    public void updateCargoPrePlayerInteraction() {
        ...
        if (this.okToUpdateShipsAndWeapons()) {
            ...
            factionPicker.add("vri_midline", 4.0f);
            ...
            this.addShips("vri_midline", 1000F, 10.0F, this.itemGenRandom.nextFloat() > pOther ? 0.0F : 10.0F, this.itemGenRandom.nextFloat() > pOther ? 0.0F : 10.0F, this.itemGenRandom.nextFloat() > pOther ? 0.0F : 10.0F, this.itemGenRandom.nextFloat() > pOther ? 0.0F : 10.0F, Math.min(1.0F, Misc.getShipQuality(this.market, this.market.getFactionId()) + 0.5F), 0.0F, FactionAPI.ShipPickMode.ALL, null, 5);
    ...
    public static void addHullsToMidlineMarket(){
        for (ShipHullSpecAPI hullSpec : Global.getSettings().getAllShipHullSpecs()) {
            if (hullSpec.isBaseHull() && !hullSpec.isDHull()) {
                if (hullSpec.getManufacturer() != null) {
                    if ((hullSpec.getManufacturer().contains("Midline")) && !hullSpec.hasTag("HIDE_IN_CODEX") && !hullSpec.hasTag(Tags.VARIANT_UNBOARDABLE)) {
                        if (!Global.getSector().getFaction("vri_midline").knowsShip(hullSpec.getHullId())) {
                            Global.getSector().getFaction("vri_midline").addKnownShip(hullSpec.getHullId(), false);
                            log.info("Added To VRI Midline dummy faction: " + hullSpec.getHullNameWithDashClass());
                        }
                    }
                }
            }
